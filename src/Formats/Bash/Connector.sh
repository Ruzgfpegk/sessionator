...
main() {
  ...
  # Populate the SERVER_INFO array from the matching line at the end of the script
  local server_info
  server_info=$(grep -o "^populate_server_info_[^[:space:]]* \"$server_name\"[^#]*" "${BASH_SOURCE[0]}" | head -n 1)
  eval "$server_info"
